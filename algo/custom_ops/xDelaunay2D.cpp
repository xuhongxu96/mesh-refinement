...
vtkStandardNewMacro(xDelaunay2D);
vtkCxxSetObjectMacro(xDelaunay2D, Transform, vtkAbstractTransform);

// Construct object with Alpha = 0.0; Tolerance = 0.00001; Offset = 1.25;
// BoundingTriangulation turned off.
xDelaunay2D::xDelaunay2D() {
  this->Alpha = 0.0;
...
//----------------------------------------------------------------------------
void xDelaunay2D::SetSourceData(vtkPolyData* input) {
  this->Superclass::SetInputData(1, input);
}

//----------------------------------------------------------------------------
// Specify the input data or filter. New style.
void xDelaunay2D::SetSourceConnection(vtkAlgorithmOutput* algOutput) {
  this->Superclass::SetInputConnection(1, algOutput);
}

vtkPolyData* xDelaunay2D::GetSource() {
...
int xDelaunay2D::InCircle(double x[3], double x1[3], double x2[3],
                          double x3[3]) {
...
vtkIdType xDelaunay2D::FindTriangle(double x[3], vtkIdType ptIds[3],
                                    vtkIdType tri, double tol, vtkIdType nei[3],
                                    vtkIdList* neighbors) {
...
// triangle id.
void xDelaunay2D::CheckEdge(vtkIdType ptId, double x[3], vtkIdType p1,
  ...
  if (numNei > 0)  // i.e., not a boundary edge
  {
    ...
    if (this->InCircle(x3, x, x1, x2)) {  // swap diagonal
...
//
int xDelaunay2D::RequestData(vtkInformation* vtkNotUsed(request),
  ...
  if (this->Transform && this->BoundingTriangulation) {
    vtkWarningMacro(
        << "Bounding triangulation cannot be used when an input transform is "
  ...
  if (this->ProjectionPlaneMode == VTK_BEST_FITTING_PLANE &&
    ...
    vtkWarningMacro(
        << "Bounding triangulation cannot be used when the best fitting plane "
           "option "
  ...
  for (ptId = 0; ptId < numPoints; ptId++) {
    ...
    if ((tri[0] = this->FindTriangle(x, pts, tri[0], tol, nei, neighbors)) >=
        0) {
  ...
  if (this->Alpha > 0.0) {
    ...
    for (i = 0; i < numTriangles; i++) {
      if (triUse[i] == 1) {
        ...
        if (triPts[0] < numPoints && triPts[1] < numPoints &&
            triPts[2] < numPoints) {
  ...
  if (!this->BoundingTriangulation && this->Alpha == 0.0 && !source) {
    ...
    for (ptId = 0; ptId < numPoints; ptId++) {
      ...
      for (i = 0; i < ncells; i++) {
        ...
        vtkDebugMacro("tri " << tri1 << " [" << triPts[0] << " " << triPts[1]
                             << " " << triPts[2] << "]");
        ...
        vtkDebugMacro("swap [" << p1 << " " << p2 << "] and [" << ptId << " "
                               << p3 << "]");
        ...
        vtkDebugMacro("candidate tri1 " << tri1 << " [" << pts[0] << " "
                                        << pts[1] << " " << pts[2] << "]"
                                        << " triUse " << triUse[tri1]);

        vtkDebugMacro("candidate tri2 " << tri2 << " [" << swapPts[0] << " "
                                        << swapPts[1] << " " << swapPts[2]
                                        << "]"
                                        << "triUse " << triUse[tri2]);
        ...
        vtkDebugMacro("replace tri1 " << tri1 << " [" << pts[0] << " " << pts[1]
                                      << " " << pts[2] << "]"
                                      << " triUse " << triUse[tri1]);

        vtkDebugMacro("replace tri2 " << tri2 << " [" << swapPts[0] << " "
                                      << swapPts[1] << " " << swapPts[2] << "]"
                                      << " triUse " << triUse[tri2]);
...
int* xDelaunay2D::RecoverBoundary(vtkPolyData* source) {
...
// What we do is identify a "submesh" of triangles that includes the edge to
// recover. Then we split the submesh in two with the recovered edge, and
// triangulate each of the two halves. If any part of this fails, we leave
// things alone.
int xDelaunay2D::RecoverEdge(vtkPolyData* source, vtkIdType p1, vtkIdType p2) {
  vtkIdType cellId = 0;
  int i, j, k;
  ...
  // Container for the edges (2 ids in a set, the order does not matter) we
  // won't check
  ...
  if (vtkMath::Normalize(splitNormal) ==
      0.0) {  // Usually means coincident points
    goto FAILURE;
  }

  // Identify a triangle connected to the point p1 containing a portion of the
  // edge.
  //
  ...
  for (i = 0; i < ncells; i++) {
    ...
    if (signX1 != signX2)  // points of triangle on either side of edge
    {
      ...
      if (vtkMath::Normalize(sepNormal) == 0.0) {  // bad mesh
        goto FAILURE;
      }
  ...
  if (i >= ncells) {  // something is really screwed up
    goto FAILURE;
  }
  ...
  while (v1 != p2) {
    ...
    if (neis->GetNumberOfIds() != 1) {  // Mesh is folded or degenerate
      goto FAILURE;
    }
    cellId = neis->GetId(0);
    ...
    for (j = 0; j < 3; j++) {
      if (pts[j] != v1 &&
          pts[j] != v2) {  // found point opposite current edge (v1,v2)
        if (pts[j] == p2) {
  ...
  if (!success) {  // polygons on either side of edge are poorly shaped
    goto FAILURE;
  }

  // Okay, delete the old triangles and replace them with new ones. There should
  // be the same number of new triangles as old ones.
  leftTris = leftPtIds->GetPointer(0);
  for (j = i = 0; i < numLeftTris; i++, j++, leftTris += 3) {
    cellId = tris->GetId(j);
    this->Mesh->RemoveCellReference(cellId);
    for (k = 0; k < 3; k++) {  // allocate new space for cell lists
      this->Mesh->ResizeCellList(leftTris[k], 1);
    }
    this->Mesh->ReplaceLinkedCell(cellId, 3, leftTris);

    // Check if the added triangle contains edges which are not in the polygon
    // edges set
    for (int e = 0; e < 3; e++) {
      ...
      if (!source->IsEdge(ep1, ep2) && !source->IsEdge(ep2, ep3) &&
          !source->IsEdge(ep3, ep1)) {
        std::set<vtkIdType> edge;
        edge.insert(ep1);
        edge.insert(ep2);
        if (polysEdges.find(edge) == polysEdges.end()) {
          // Add this new edge and remember current triangle and third point ids
          // too
...
void xDelaunay2D::FillPolygons(vtkCellArray* polys, int* triUse) {
  ...
  // Loop over edges of polygon, marking triangles on "outside" of polygon as
  // outside. Then perform a fill.
  for (polys->InitTraversal(); polys->GetNextCell(npts, pts);) {
    currentFront->Reset();
    for (i = 0; i < npts; i++) {
      p1 = pts[i];
      ...
      } else  // Mark the "outside" triangles
      {
        ...
        this->Mesh->GetCellEdgeNeighbors(-1, p1, p2,
                                         neis);  // get both triangles
        numNeis = neis->GetNumberOfIds();
        for (j = 0; j < numNeis; j++) {  // find the vertex not on the edge;
                                         // evaluate it (and the cell) in/out
          cellId = neis->GetId(j);
...
//----------------------------------------------------------------------------
int xDelaunay2D::FillInputPortInformation(int port, vtkInformation* info) {
  if (port == 0) {
...
//----------------------------------------------------------------------------
vtkAbstractTransform* xDelaunay2D::ComputeBestFittingPlane(vtkPointSet* input) {
  ...
  c3 = m + 6;
  if (!normal_computed &&
      (det = vtkMath::Determinant3x3(c1, c2, c3)) > tolerance) {
...
//----------------------------------------------------------------------------
void xDelaunay2D::PrintSelf(ostream& os, vtkIndent indent) {
  this->Superclass::PrintSelf(os, indent);

  os << indent << "Alpha: " << this->Alpha << "\n";
  os << indent << "ProjectionPlaneMode: "
     << ((this->ProjectionPlaneMode == VTK_BEST_FITTING_PLANE)
             ? "Best Fitting Plane"
             : "XY Plane")
     << "\n";
